...
def read_save(save_path, main_path, chart_name="TGA_chart", file_type="TXT"):
    paths = glob.glob(f"{main_path}\\*.{file_type}")
    columns=["Sample", "Temperature (°C)", "Relative Mass (%)"]
        ...
        with open(f) as file:
            ...
            for row in txt[4:]:
                if row != "\n":
                    ...
                    list_temp.append([filename, float(row_temp[4]), float(row_temp[3])])
